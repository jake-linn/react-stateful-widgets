...
const [programerId, setProgramerId] = useState(null) 
  ...
  const getNameOfFeatured = () => {
    ...
    const id = programerId -1;
    const name = programmer[id].name
    return name 
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programerId ? 'gold':  'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmer.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setProgramerId(dev.id)}>Feature</button>
            </div>
          )
        }
      </div>
